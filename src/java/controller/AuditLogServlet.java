...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    try (Connection conn = DBConnection.getConnection()) {
        ...
        if ("listByUser".equals(action)) {
            ...
            List<model.Session> sessions = sessionDAO.getSessionsByUser(userId);
            ...
            for (AuditLog log : logs) {
                for (model.Session session : sessions) {
                    if (log.getUserId() == session.getUserId() &&
                        log.getIpAddress().equals(session.getIpAddress())) {
        ...
        Logger.getLogger(AuditLogServlet.class.getName()).log(Level.SEVERE, null, ex);
