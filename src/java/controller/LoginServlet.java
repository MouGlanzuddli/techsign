            ...
            try {
                ...
                if (rs.next()) {
                    ...
                    if (user != null) {
                        ...
                        if (roleId == 2) {
                            try {
                                String profileQuery = "SELECT id FROM candidate_profiles WHERE user_id = ?";
                                PreparedStatement psProfile = conn.prepareStatement(profileQuery);
                                psProfile.setInt(1, user.getId());
                                ResultSet rsProfile = psProfile.executeQuery();
                                if (rsProfile.next()) {
                                    int profileId = rsProfile.getInt("id");
                                    session.setAttribute("candidate_profile_id", profileId);
                                    System.out.println("DEBUG - Set candidate_profile_id = " + profileId);
                                } else {
                                    System.out.println("DEBUG - Không tìm thấy profile cho user_id = " + user.getId());
                                }
                                rsProfile.close();
                                psProfile.close();
