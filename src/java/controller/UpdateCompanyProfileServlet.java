...
@WebServlet(name = "UpdateCompanyProfileServlet", urlPatterns = {"/UpdateCompanyProfileServlet"})
@MultipartConfig
public class UpdateCompanyProfileServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // Get all non-file form parameters
        Collection<Part> parts = request.getParts();
        for (Part part : parts) {
            if (part.getSubmittedFileName() == null) { // Regular form field, not a file upload
                String fieldName = part.getName();
                String fieldValue = getFieldValue(part);
        ...
        try {
            ...
            if (avatarPart != null && avatarPart.getSize() > 0) {
                ...
                if (!ext.equals(".jpg") && !ext.equals(".jpeg") && !ext.equals(".png")) {
                    errorMsg = "Only jpg, jpeg, png images are accepted!";
                } else if (avatarPart.getSize() > 2 * 1024 * 1024) {
                    errorMsg = "Image cannot be larger than 2MB!";
                } else {
            ...
            ex.printStackTrace();
            errorMsg = "Error uploading image: " + ex.getMessage();
        }
        ...
        try {
            ...
            // Cập nhật user
            user.setFullName(companyName); // full_name là companyName
            user.setEmail(email);
            user.setPhone(phone);
    ...
    // Helper method to read form field values from Parts
    private String getFieldValue(Part part) {
