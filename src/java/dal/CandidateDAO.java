...
public class CandidateDAO {
    ...
    // Lấy candidate theo user_id, lấy thêm email từ bảng users nếu có
    ...
    // Cập nhật profile candidate (phiên bản đơn giản, có trường resumeUrl)
    public boolean updateCandidateProfile(int userId, String jobTitle, int experienceYears, String educationLevel, String address, String resumeUrl) throws SQLException {
        String sql = "UPDATE candidate_profiles SET job_title = ?, experience_years = ?, education_level = ?, address = ?, resume_url = ?, updated_at = ? WHERE user_id = ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, jobTitle);
            stmt.setInt(2, experienceYears);
            stmt.setString(3, educationLevel);
            stmt.setString(4, address);
            stmt.setString(5, resumeUrl);
    ...
    public List<Candidate> searchCandidates(String location, String skills, Integer minExperience) throws SQLException {
        ...
        // Nếu có trường skills thì thêm vào đây nếu model có
    ...
    private Candidate mapResultSetToCandidate(ResultSet rs) throws SQLException {
        ...
        // Không lấy full_name, email, phone từ candidate_profiles nữa
