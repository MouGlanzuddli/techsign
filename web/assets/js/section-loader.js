    ...
    loadSectionWithData(file, sectionId, breadcrumb) {
        ...
        fetch(jspUrl)
            ...
            .then(jspContent => {
                ...
                if (section) {
                    document.getElementById('section-content').innerHTML = section.outerHTML;
                    console.log('Section content loaded');
                    // Re-initialize Bootstrap tabs for dynamically loaded content
                    document.querySelectorAll('#notificationTabs button[data-bs-toggle="tab"]').forEach(function(tabEl) {
                        new bootstrap.Tab(tabEl);
                    });
                    // Optionally, activate the first tab
                    var firstTab = document.querySelector('#notificationTabs button[data-bs-toggle="tab"]');
                    if (firstTab) {
                        var tab = new bootstrap.Tab(firstTab);
                        tab.show();
                    }
                    attachModalListeners();
                    // FIX: Only call loadCompanies after company-jobs section is loaded
                    if (sectionId === 'company-jobs' && typeof window.loadCompanies === 'function') {
