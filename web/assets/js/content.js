    ...
    filteredPosts.forEach(post => {
        ...
        if (post.status === 'pending') {
            actionButtons = `
                ...
                <button class="btn btn-filled" onclick="event.stopPropagation();rejectPost(${post.id})">Reject</button>
            `;
        }
        actionButtons += `<button class="btn btn-delete" onclick="event.stopPropagation();deletePost(${post.id})" title="Xóa"><i class="fas fa-trash"></i></button>`;
        html += `
        <div class="job-card">
